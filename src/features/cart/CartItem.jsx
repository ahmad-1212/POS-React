...
const CartItem = ({ itm }) => {
  ...
  const img = CATEGORIES.find(
    (cat) => cat.name.toLowerCase() === itm.name.toLowerCase()
  )?.image;
  return (
    <li className="flex items-center bg-gray-100 rounded-md py-2 px-3 gap-3">
      ...
      <div className="flex flex-col justify-between flex-1">
        <h3 className="font-[600] capitalize">{itm.name}</h3>
        <div className="flex items-center justify-between">
          <h5 className="font-[700] text-primary-500">
            ${itm.price.toFixed(2, 0)}
          </h5>
          <div className="flex items-center gap-2">
            <button
              onClick={() => dispatch(decreaseItemQuantity(itm.name))}
            ...
            <button
              onClick={() => dispatch(increaseItemQuantity(itm.name))}
              className="border-2 rounded-md px-2 border-primary-500 hover:bg-primary-500 hover:text-primary-100"
            >
              +
            </button>
